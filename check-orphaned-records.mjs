...
function loadEnvVars() {
  ...
  // Carregar .env.save (sobrescreve .env.local)
  const envSavePath = path.join(__dirname, '.env.save');
  if (fs.existsSync(envSavePath)) {
    const envSaveContent = fs.readFileSync(envSavePath, 'utf8');
    envSaveContent.split('\n').forEach(line => {
      const [key, value] = line.split('=');
...
const EVOLUTION_API_URL = envVars.VITE_EVOLUTION_API_URL || envVars.EVOLUTION_API_URL;
const EVOLUTION_API_KEY = envVars.VITE_EVOLUTION_API_KEY || envVars.EVOLUTION_API_KEY;
